import { validateImageUrl, escapeCssUrl } from '../utils/validation'
...
export default function PromoBanner({
  ...
  // Sanitize image URL to prevent XSS and CSS injection
  const validUrl = validateImageUrl(backgroundImage).isValid ? backgroundImage : DEFAULT_IMAGE
  const safeImageUrl = escapeCssUrl(validUrl)

  return (
    <div className="px-4 sm:px-6 md:px-8 lg:px-12 mb-6">
      <div className="max-w-7xl mx-auto">
        <div
          className="relative h-40 sm:h-48 md:h-56 lg:h-64 rounded-2xl overflow-hidden"
          style={{
